...
def run_simulation(strength=STRENGTH, steps=NUMBER_OF_STEPS, V0=V0, width=INITIAL_WIDTH, initial_position=INITIAL_POSITION, num_points=NUM_POINTS, box_size=BOX_SIZE):
    # could add initial position and width as parameters, but for now just use the constants defined in config.py
...
'''
    ...
    percent_error = calculate_percent_error(T, calculate_theoretical_transmission(V0, E0))

    return x_axis, current_packet, T, E0, kappa, kappa_L, percent_error

best_params = np.zeros(3)        # strength, steps, V0
    ...
    for k in steps:
        for n in num_points:
            for bx in box_sizes:
                ...
                x_axis, current_packet, T, E0, kappa, kappa_L, percent_error = run_simulation(strength=s, steps=k, num_points=n, box_size=bx)
