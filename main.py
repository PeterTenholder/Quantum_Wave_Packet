...
psi_new = normalized_packet * np.exp(-1 * np.sqrt(-1) * V * dt / (2 * HBAR))
...
phi_new_drifted = phi_new * np.exp(-1 * np.sqrt(-1) * HBAR * (PX**2 + PY**2 + PZ**2) * dt / (2 * MASS)) # k^2 = px^2 + py^2 + pz^2
...
psi_final = psi_new_drifted * np.exp(-1 * np.sqrt(-1) * V * dt / (2 * HBAR))